<x-themequiz title="">
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">คำสั่งซื้อของฉัน</div>
                    <div class="card-body">
                                        ...
                                        <tr>
                                                ...
                                                @switch($item->status)
                                                    @case('created')
                                                        ...
                                                        <a class="btn btn-sm btn-warning"
                                                            href="{{ url('payment/create?order_id=' . $item->id) }}">ส่งหลักฐาน</a>
                                                    @break

                                                    @case('checking')
                                                        <div>รอตรวจสอบ</div>
                                                        <div><img src="{{ asset("storage/{$item->payment->slip}") }}"
                                                                width="100" /></div>
                                                        @if (Auth::user()->role == 'admin')
                                                            <form method="POST" action="{{ url('/order/' . $item->id) }}"
                                                                ...
                                                                <button class="btn btn-primary btn-sm"
                                                                    type="submit">เปลี่ยนสถานะ</button>
                                                            </form>
                                                        @endif
                                                    @break
                                                    ...
                                                    @case('paid')
                                                        ...
                                                        @if (Auth::user()->role == 'admin')
                                                            <form method="POST" action="{{ url('/order/' . $item->id) }}"
                                                                ...
                                                                {{ method_field('PATCH') }}
                                                                {{ csrf_field() }}
                                                                <input name="tracking" type="text" id="tracking" value=""
                                                                    placeholder="ใส่เลข tracking...">
                                                                <input name="status" type="hidden" id="status"
                                                                    value="completed">
                                                                <button class="btn btn-primary btn-sm" type="submit">ส่งเลข
                                                                    Tracking</button>
                                                            </form>
                                                        @endif
                                                    @break
                                            ...
                                            <td>
                                                ...
                                                <a href="{{ url('/order/' . $item->id . '/edit') }}"
                                                    title="Edit Order"><button class="btn btn-primary btn-sm"><i
